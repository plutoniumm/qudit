...
class CodeOrthonormality(Question):
    ...
    def test_dutta3_orthonormal(self):
        ...
        self.assertAlmostEqual(
            pt.dot(s0, s0).real.item(), 1.0, places=6, msg="Dutta3 |0_L> not normalized"
        )

        self.assertAlmostEqual(
            pt.dot(s1, s1).real.item(), 1.0, places=6, msg="Dutta3 |1_L> not normalized"
        )

        self.assertAlmostEqual(
            pt.abs(pt.dot(s0, s1)).item(),
            0.0,
            places=6,
            msg="Dutta3 codewords not orthogonal",
        )
    ...
    def test_perfect_orthonormal(self):
        ...
        self.assertAlmostEqual(
            pt.dot(s0, s0).real.item(),
            1.0,
            places=5,
            msg="Perfect |0_L> not normalized",
        )

        self.assertAlmostEqual(
            pt.dot(s1, s1).real.item(),
            1.0,
            places=5,
            msg="Perfect |1_L> not normalized",
        )
        ...
        self.assertAlmostEqual(
            ...
            0.0,
            places=5,
            msg="Perfect codewords not orthogonal",
        )
    ...
    def test_perfect_norm_each(self):
        ...
        for cw in code:
            self.assertAlmostEqual(
                pt.linalg.norm(cw.float()).item(),
                1.0,
                places=5,
                msg="Each Perfect codeword should have unit norm",
            )
    ...
    def test_dutta3_span_3qubit(self):
        ...
        """
        self.assertEqual(
            Dutta3().toTensor().shape[1],
            8,
            msg="Dutta3 should live in 2^3=8-dimensional space",
        )
    ...
    def test_perfect_span_5qubit(self):
        ...
        """
        self.assertEqual(
            Perfect().toTensor().shape[1],
            32,
            msg="Perfect code should live in 2^5=32-dimensional space",
        )
    ...
    def test_from_stabilizers_3qubit_bitflip(self):
        ...
        self.assertEqual(
            cws.shape[0], 2, msg="3-qubit bit-flip code should have 2 codewords"
        )

        self.assertEqual(
            cws.shape[1], 8, msg="3-qubit code codewords should be in 2^3=8-dim space"
        )
        ...
        for n in norms:
            self.assertAlmostEqual(
                ...
                1.0,
                places=5,
                msg="3-qubit bit-flip codewords should be normalized",
            )
        inner = pt.abs(cws[0] @ cws[1].conj()).item()

        self.assertAlmostEqual(
            inner, 0.0, places=5, msg="3-qubit bit-flip codewords should be orthogonal"
        )
    ...
    def test_from_stabilizers_codewords_are_000_111(self):
        ...
        for cw in cws:
            ...
            self.assertEqual(
                nonzero,
                1,
                msg="3-qubit bit-flip codeword should be a computational basis state",
            )
        ...
        self.assertEqual(
            indices,
            {0, 7},
            msg="Codewords should be |000> (index 0) and |111> (index 7)",
        )
